...
class DrivableMap {
    ...
    public boolean hasFasterThan(int aSpeed) {
        boolean hasFaster = false;
        for (Drivable drivableValue : drivable_map.values()) {
            if (drivableValue.getMaxSpeed() >= aSpeed) {
                hasFaster = true;
            }
        }
        return hasFaster;
    ...
    public List<Tradable> getTradable() {
        ...
        for (Drivable drivable : drivable_map.values()) {
            if (drivable instanceof Tradable) {
                Tradable tradable = (Tradable) drivable;
                tradableItems.add(tradable);
            }
        }
